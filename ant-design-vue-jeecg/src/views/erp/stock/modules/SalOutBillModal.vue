    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-form :form="form">
        ...
        <a-row>
          ...
          <a-col :span="8">
            <a-form-item label="结算数量是否等于出库" :labelCol="{span:15}" :wrapperCol="{span:9}">
              <j-dict-select-tag  :disabled="readOnly"
                                  @change="onIsSameSettleChange"
                                  type="list"
    ...
    methods: {
      ...
      onIsSameSettleChange(val) {
        ...
        if (val === '1') {
          ...
          for (let v of values) {
            this.$refs.stkIoBillEntry.setValues([{rowKey: v.id, values: {settleQty: v.qty}}]);
          }
        }
      },
